...
## What it does
Generates buy/hold signals based on a simple moving average crossover and will eventually simulate portfolios against those signals. It also creates a local database to store your stock prices based on user preferences, so you can reuse previously downloaded historical data locally without requiring Yahoo Finance.

## Usage
Before running the program, you need to initialize the database that stores stock prices by running the following lines:
```py
import data_loader
...
---
To download and save data to the database, run the command with your chosen ticker; this will insert/replace data from yfinance in the database for that ticker.
